...
public class GeneticAlgorithm {
    ...
    public GeneticAlgorithm(DataForSchedule dataForSchedule) {
    ...
    Population crossoverPopulation(Population population) {
        Population crossoverPopulation = new Population(population.getSchedules().size(), dataForSchedule);
        IntStream.range(Driver.NUMB_OF_ELITE_SCHEDULES, population.getSchedules().size())
                .forEach(x -> {
                    if (Driver.CROSSOVER_RATE > Math.random()) {
                        // Lấy 2 cá thể có độ fitness lớn nhất để lai với nhau
    ...
    Schedule crossoverSchedule(Schedule schedule1, Schedule schedule2) {
        Schedule crossoverSchedule = new Schedule(dataForSchedule).initialize();
    ...
    Population mutatePopulation(Population population) {
        Population mutatePopulation = new Population(population.getSchedules().size(), dataForSchedule);
    ...
    Schedule mutateSchedule(Schedule mutateSchedule) {
        Schedule schedule = new Schedule(dataForSchedule).initialize();
    ...
    Population selectTournamentPopulation(Population population) {
        Population tournamentPopulation = new Population(Driver.TOURNAMENT_SELECTION_SIZE, dataForSchedule);
